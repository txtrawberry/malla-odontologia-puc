// Definición completa de la malla de Odontología PUC basada en la documentación oficial
const semestersData = [
    {
        id: 1,
        title: "1° Semestre",
    ...
    semestersData.forEach(sem => {
        ...
        sem.courses.forEach(course => {
            ...
            card.addEventListener('click', () => {
                if (isApproved) {
                    // Desaprobar (y remover en cascada si es necesario, o simplemente quitar)
                    approvedCourses = approvedCourses.filter(s => s !== course.sigla);
                } else if (isAvailable) {
                    // Aprobar
                    approvedCourses.push(course.sigla);
                } else {
                    // Está bloqueado, mostrar pequeño aviso visual o ignorar
